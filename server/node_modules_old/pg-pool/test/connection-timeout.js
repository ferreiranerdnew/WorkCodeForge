...
describe('connection timeout', () => {
  ...
  it('should callback with an error if timeout is passed', (done) => {
    const pool = new Pool({ connectionTimeoutMillis: 10, port: this.port, host: '192.168.110.178' })
  ...
  it(
    ...
    co.wrap(
      function* () {
        const errors = []
        const pool = new Pool({ connectionTimeoutMillis: 1, port: this.port, host: '192.168.110.178' })
        for (var i = 0; i < 15; i++) {
          try {
            yield pool.connect()
          } catch (e) {
